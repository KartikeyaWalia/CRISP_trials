...
{
  dynamixel_state_list_pub_ = priv_node_handle_.advertise<dynamixel_workbench_msgs::DynamixelStateList>("dynamixel_state", 100); // dynamixel states pretty
  if (is_joint_state_topic_) joint_states_pub_ = priv_node_handle_.advertise<sensor_msgs::JointState>("joint_states", 100); // ros version of dynamixel states
}

void DynamixelController::initSubscriber()
{
  trajectory_sub_ = priv_node_handle_.subscribe("joint_trajectory", 100, &DynamixelController::trajectoryMsgCallback, this); //Receives joint trajectory to control DYNAMIXEL
...
{
  dynamixel_command_server_ = priv_node_handle_.advertiseService("dynamixel_command", &DynamixelController::dynamixelCommandMsgCallback, this); //Receives command to control DYNAMIXEL
                                                      ...
                                                      dynamixel_workbench_msgs::DynamixelCommand::Response &res)
{
  ...
  result = dxl_wb_->itemWrite(id, item_name.c_str(), value, &log); //sending packet
  if (result == false)
  {
    ROS_ERROR("%s", log);
