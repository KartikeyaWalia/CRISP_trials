...
int main(int argc, char *argv[]) 
{
  ...
  DynamixelWorkbench dxl_wb; // dxl_wb is an object of the Dynamixel Workbench class (an API by robotis)

  const char *log; // ?
  bool result = false;

  uint8_t scanned_id[100]; //maximum ids to scan
  uint8_t dxl_cnt = 0;

  // uint32_t baudrate[BAUDRATE_NUM] = {9600, 57600, 115200, 1000000, 2000000, 3000000, 4000000};
  uint32_t baudrate[BAUDRATE_NUM] = {3000000};
  uint8_t range = 253;
  ...
  while (index < BAUDRATE_NUM)
  {
    ...
    else
    {
      ...
      {
        ROS_INFO("Found %d Dynamixels", dxl_cnt);
