#include "crisp_arm_core/trajectory_server.hpp"
#include <array>
#include <numeric>
#include <vector>
...
DynamixelTrajectoryAction::DynamixelTrajectoryAction(const std::string &name)
    : as_(nh_, name,boost::bind(&DynamixelTrajectoryAction::executeCB, this, _1), false), action_name_(name) 
{
  jointPub = nh_.advertise<trajectory_msgs::JointTrajectory>("/dynamixel_workbench/joint_trajectory", 100);
...
void DynamixelTrajectoryAction::executeCB(const control_msgs::FollowJointTrajectoryGoalConstPtr &goal) 
{
  ...
  targetState.points = goal->trajectory.points; //sends: positions, velocities, accelerations, effort, time_from_start
  targetState.header = goal->trajectory.header;
  targetState.joint_names = goal->trajectory.joint_names;

  ros::Rate rate(250); // 250hz 4ms set in dynamixel workbench,, 1000hz: 1ms
  ...
  //   std::vector<double> curPointPositions = calculatePoint(nowFromStart);
    ...
    ROS_INFO("targetState");
    targetState.header.stamp = ros::Time::now();
    jointPub.publish(targetState);


    // feedback_.desired           = points[idx];
    // feedback_.actual.positions  = targetState.points.positions;
    // feedback_.actual.velocities = targetState.points.velocities;
    // feedback_.actual.effort     = targetState.points.efforts;
...
int main(int argc, char **argv) 
{
  ...
  DynamixelTrajectoryAction action("crisp_arm/follow_joint_trajectory"); //namespace
